...
└── [dataset_name]/          # Extracted dataset folder
    ...
    │   └── item_cf_embeds_large3_array.npy # Item lanuage representation 
...
* **Requirement**: Only the `train.txt`, `valid.txt`, and `test.txt` files under `cf_data/`, along with `item_cf_embeds_<Language_Model>_array.npy` under `item_info/`, are mandatory.
* **Automation**: You **do not need** to manually build the `.npz` or `.npy` files. The preprocessing pipeline will automatically generate all artifacts in `item_info/` and `preprocessed/` using the source data from `cf_data/` and `item_info/`.
