...
class AlphaFreeRecDemo(torch.nn.Module):
    """
        ...
        This class only supports the amazon_video dataset, with a hard-coded number of items.
        For other datasets, please modify the number of items and the loading of item_cf_embeds accordingly.
    """
    def __init__(self, args) -> None:
        ...
        self.n_items = 25612 # default for amazon_video
    ...
    def predict(self, interactions, items=None):
        """
        interactions: list[int] of interacted item ids
        items: list[int] candidate item ids; if None, consider all items
        return: top-20 item indices in the candidate list space (or global if items is None)
        """
        if items is None:
            items = list(range(self.n_items))
        ...
        items_tensor = torch.tensor(items, device=self.device, dtype=torch.long)
        items_emb = all_items[items_tensor]

        users = F.normalize(users, dim=-1)
        items_emb = F.normalize(items_emb, dim=-1)

        # scores: (batch, n_candidates)
        rate_batch = users @ items_emb.t()

        # --------- mask already interacted items ----------
        # Map interacted item ids -> positions in candidate list
        interacted_set = set(int(x) for x in interactions)

        if interacted_set:
            # Build a mask over candidates
            cand_ids = items_tensor.detach().cpu().tolist()
            bad_pos = [i for i, iid in enumerate(cand_ids) if iid in interacted_set]

            if bad_pos:
                bad_pos = torch.tensor(bad_pos, device=self.device, dtype=torch.long)
                rate_batch.index_fill_(dim=1, index=bad_pos, value=float("-inf"))
        # -----------------------------------------------

        return torch.topk(rate_batch, k=20, dim=1)[1]